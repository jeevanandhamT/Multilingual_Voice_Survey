...
<body>
    <div class="container">
        ...
        <div class="cont-bg">
            ...
            <div class="bttn"><a href="{{ url_for('app4_index') }}"> <button class="btn"lang="{{ lang_code }}">Next</button></a></div>
        </div>
    </div>
    <div>
            Language:
...
<body>
    <div class="container">
        ...
        <div class="cont-bg">
            <div class="cont" lang="{{ lang_code }}">4. We would love your feedback. Leave your coments here. </div>
            <div class="ipt"><input type="text" class="txt"><input type="image" src="{{ url_for('static', filename='micro.svg') }}" alt="Submit" id="imageBtn"></div>
            <div class="bttn"><a href="{{ url_for('app4_index') }}"> <button class="btn"lang="{{ lang_code }}">Next</button></a></div>
        </div>
    </div>
...
def app1_index():
    ...
    if request.method == 'POST':
        if 'app2' in request.referrer:  # Check if the request came from app2 page
            recognized_text = request.form.get('textbox1', '')
        else:
            recognized_text = recognize_and_translate_to_english()
    ...
    if a == 2:
        return render_template_string(html_template2, lang_code=lang_code, default_value=recognized_text)
    else:
        return render_template_string(html_template1, lang_code=lang_code, default_value=recognized_text)
...
def app3_index():
    ...
    if request.method == 'POST':
        ...
        recognized_text = None
    return render_template_string(html_template3, lang_code='en', default_value=recognized_text)

@app.route('/next-to-app3', methods=['POST'])
def next_to_app3():
    recognized_text = request.form.get('textbox1', '')
    return redirect(url_for('app4_index', recognized_text=recognized_text))

@app.route('/app4', methods=['GET', 'POST'])
def app4_index():
    # Route for the fourth page
    if request.method == 'POST':
        recognized_text = recognize_and_translate_to_english()
    else:
        recognized_text = None
    return render_template_string(html_template4, lang_code='en', default_value=recognized_text)

@app.route('/next-to-app4', methods=['POST'])
def next_to_app4():
    recognized_text = request.form.get('textbox4', '')
    # You can perform any processing or data storage here if needed before redirecting
    return redirect(url_for('app4_index', recognized_text=recognized_text))
